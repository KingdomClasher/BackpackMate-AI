...
Response Reception: The system receives the LLM response and checks for structured objects in the object field
Type Detection: If an object exists, the system examines its type field
...
Processing: If validation passes (or no validation exists), the processor’s execute function is called
