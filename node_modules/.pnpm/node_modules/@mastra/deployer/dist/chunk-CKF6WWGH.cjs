...
var Bundler = class extends bundler.MastraBundler {
  ...
  async writeInstrumentationFile(outputDirectory, customInstrumentationFile) {
    ...
    const __dirname = path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-CKF6WWGH.cjs', document.baseURI).href))));
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkBZDTNUHJ_cjs.createBundler(inputOptions, outputOptions);
  }
  async analyze(entry, mastraFile, outputDirectory, { enableEsmShim = true } = {}) {
    return await chunk36EU6KMX_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async installDependencies(outputDirectory, rootDir = process.cwd()) {
    const deps = new chunkCGLMV75H_cjs.DepsService(rootDir);
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap = false, enableEsmShim = true } = {}) {
    const { workspaceRoot } = await chunk36EU6KMX_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    for (const toolPath of toolsPaths) {
      ...
      for (const path$1 of expandedPaths) {
        if (await fsExtra__default.default.pathExists(path$1)) {
          const fileService = new chunkCGLMV75H_cjs.FileService();
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    let sourcemap = false;
    try {
      const bundlerOptions = await chunk36EU6KMX_cjs.getBundlerOptions(mastraEntryFile, analyzeDir);
    ...
    try {
      ...
      analyzedBundleInfo = await chunk36EU6KMX_cjs.analyzeBundle(
    ...
    try {
      const result = await chunkAGJGRBDD_cjs.writeTelemetryConfig(mastraEntryFile, path.join(outputDirectory, this.outputDir));
    ...
    for (const dep of analyzedBundleInfo.externalDependencies) {
      try {
        ...
        const rootPath = await chunk36EU6KMX_cjs.getPackageRootPath(dep);
        const pkg2 = await fsExtra.readJSON(`${rootPath}/package.json`);
        dependenciesToInstall.set(dep, pkg2.version);
      } catch {
    ...
    if (workspaceDependencies.size > 0) {
      try {
        const result = chunk36EU6KMX_cjs.collectTransitiveWorkspaceDependencies({
        ...
        await chunk36EU6KMX_cjs.packWorkspaceDependencies({
          workspaceMap: analyzedBundleInfo.workspaceMap,
...
//# sourceMappingURL=chunk-CKF6WWGH.cjs.map
//# sourceMappingURL=chunk-CKF6WWGH.cjs.map