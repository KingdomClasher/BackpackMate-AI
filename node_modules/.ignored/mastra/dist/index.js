#! /usr/bin/env node
import { create, checkForPkgJson, checkAndInstallCoreDeps, interactivePrompt, init, logger, FileService, DepsService } from './chunk-ZSVZMJ6K.js';
export { create } from './chunk-ZSVZMJ6K.js';
...
program.command("create [project-name]").description("Create a new Mastra project").option("--default", "Quick start with defaults(src, OpenAI, examples)").option("-c, --components <components>", "Comma-separated list of components (agents, tools, workflows)").option("-l, --llm <model-provider>", "Default model provider (openai, anthropic, groq, google, cerebras or mistral)").option("-k, --llm-api-key <api-key>", "API key for the model provider").option("-e, --example", "Include example code").option("-n, --no-example", "Do not include example code").option("-t, --timeout [timeout]", "Configurable timeout for package installation, defaults to 60000 ms").option("-d, --dir <directory>", "Target directory for Mastra source code (default: src/)").option(
...
).option("-s, --https", "Enable local HTTPS").action(startDevServer);
