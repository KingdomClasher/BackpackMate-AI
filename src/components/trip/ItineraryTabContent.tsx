...
const HOUR_HEIGHT = 60; // px per hour for calendar layout
      ...
      >
        ...
        {events.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="absolute rounded-lg border border-slate-300 bg-white/90 px-3 py-2 shadow-sm"
              style={{
                top: `${startOffset}%`,
                height: `${duration}%`,
                left: `${left}%`,
...
const DayListPanel = ({ day }: { day: ItineraryDay }) => (
...
export function ItineraryTabContent({ tripData }: ItineraryTabContentProps) {
  ...
  const tripName = useMemo(() => {
    const destinations = tripData?.answers?.destinations || [];
    if (Array.isArray(destinations) && destinations.length > 0) {
      return destinations.join(" • ");
    }
    return tripData?.proposedItinerary?.summary || undefined;
  }, [tripData?.answers?.destinations, tripData?.proposedItinerary?.summary]);
  ...
  if (itinerary.length === 0) {
    return (
      <div className="flex min-h-[320px] items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-sm text-slate-500">
        Your itinerary is being generated. Hang tight!
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Trip itinerary</h2>
          <p className="text-sm text-slate-500">
            Switch between a timeline calendar or a focused list for each day.
          </p>
        </div>
      ...
      {viewMode === "calendar" ? (
        <div className="overflow-x-auto pb-2">
          <div className="flex min-w-full gap-4">
            {itinerary.map((day, index) => (
              <DayCalendarColumn key={day.id ?? index} day={day} index={index} />
            ))}
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <DayListPanel day={itinerary[activeDayIndex]} />
        </div>
      )}
